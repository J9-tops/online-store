...
import { useProducts, useSearchProducts } from "@/services/product";
...
export default function ProductDataBar() {
  ...
  const products = useMemo(() => {
    ...
    return allProducts || [];
  }, [debouncedSearchTerm, searchResults, allProducts]);
      ...
      <div className="relative">
        <div className="relative">
          ...
          <input
            ...
            value={searchTerm}
            onChange={(e) => handleSearchChange(e.target.value)}
            className="w-full pl-10 pr-10 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200"
          />
          {searchTerm && (
            <button
              onClick={clearSearch}
        ...
        {isSearching && products.length > 0 && !isWaitingForDebounce && (
